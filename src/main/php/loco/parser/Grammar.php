...
class Grammar extends MonoParser {
	...
	/**
	 * @param string                $root              All parsing begins with the parser of this name.
	 * @param MonoParser[]|string[] $internals
	 * @param callable              $callback
	 */
	public function __construct($root, $internals, $callback = null) {
		parent::__construct($internals, $callback);

		if(!array_key_exists($root, $this->internals)) {
			throw new GrammarException("This grammar begins with rule '".var_export($root, true)."' but no parser with this name was given.");
		}
		$this->root = $root;
	...
	public function getResult($string, $i = 0) {
		$match = $this->internals[$this->root]->match($string, $i);
		return array(
			"j" => $match["j"],
	...
	public function evaluateNullability() {
		return ($this->internals[$this->root]->nullable === true);
	}

	// S is the first
	public function firstSet() {
		return array($this->internals[$this->root]);
	}
}