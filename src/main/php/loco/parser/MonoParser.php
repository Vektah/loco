...
abstract class MonoParser {
	 ...
	 *
	 * TODO: Yuck why is this public?
	 *
	 * @var MonoParser[]|string[]
	 */
	protected $internals;
	...
	public function __construct($internals, $callback) {
		...
		if(!is_array($internals)) {
			throw new GrammarException(print_r($internals, true)." should be an array");
		}

		foreach($internals as $internal) {
			if(!is_string($internal) && !($internal instanceof self)) {
				throw new GrammarException(print_r($internal, true)." should be either a string or a MonoParser");
			}
		}
	 ...
	 * @param string    $string     The string being matched
	 * @param int       $i          offset within string
	 *
	 * @return array
	 */
